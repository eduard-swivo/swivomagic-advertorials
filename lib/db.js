...
export async function initDatabase() {
    ...
    `;
        ...
        try {
            ...
            await sql`ALTER TABLE articles ADD COLUMN IF NOT EXISTS product_images JSONB DEFAULT '[]';`;
            ...
            await sql`ALTER TABLE articles ADD COLUMN IF NOT EXISTS visual_brief TEXT;`; // Add visual brief
        } catch (e) {
...
// Get article by slug (Admin - includes unpublished)
export async function getArticleBySlugForAdmin(slug) {
    try {
        const { rows } = await sql`
      SELECT * FROM articles 
      WHERE slug = ${slug}
...
export async function createArticle(articleData) {
    try {
      ...
      INSERT INTO articles (
        ...
        hero_image, second_image, product_main_image, product_images, advertorial_label, hook, story, benefits,
        urgency_box, comments, cta_link, cta_text, countdown_timer, visual_brief, published
      ) VALUES (
        ${slug}, ${title}, ${category}, ${author}, ${published_date},
        ${excerpt}, ${hero_image}, ${second_image}, ${product_main_image || null}, ${JSON.stringify(product_images || [])}, ${advertorial_label}, ${hook},
        ${JSON.stringify(story)}, ${JSON.stringify(benefits)},
        ${JSON.stringify(urgency_box)}, ${JSON.stringify(comments || [])}, 
        ${cta_link}, ${cta_text}, ${JSON.stringify(countdown_timer || { enabled: false, minutes: 20 })}, ${visual_brief || null}, ${published}
      )
      RETURNING *
    `;
...
export async function updateArticle(slug, articleData) {
    try {
      ...
      UPDATE articles SET
        ...
        product_main_image = ${product_main_image || null},
...
export async function getPersonas() {
    try {
        const { rows } = await sql`SELECT * FROM personas ORDER BY created_at DESC`;
        return rows;
    } catch (error) {
...
export async function createPersona(name, description) {
    try {
        const { rows } = await sql`
