...
export async function initDatabase() {
    ...
    `;
        ...
        try {
            ...
            console.log('Migration note: columns might already exist or error:', e.message);
        }
...
export async function createArticle(articleData) {
    try {
      ...
      INSERT INTO articles (
        ...
        hero_image, second_image, advertorial_label, hook, story, benefits,
        urgency_box, comments, cta_link, cta_text, published
      ) VALUES (
        ${slug}, ${title}, ${category}, ${author}, ${published_date},
        ${excerpt}, ${hero_image}, ${second_image}, ${advertorial_label}, ${hook},
...
export async function createProduct(productData) {
    const { name, url, images, description } = productData;
    try {
        const { rows } = await sql`
            INSERT INTO product_profiles (name, url, images, description)
            VALUES (${name}, ${url}, ${JSON.stringify(images)}, ${description})
            RETURNING *
        `;
        return rows[0];
    } catch (error) {
